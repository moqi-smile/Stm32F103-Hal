...
void SX1276LoRaGetRxPacket( void *buffer, uint16_t *size )
{
    *size = RxPacketSize;
    RxPacketSize = 0;
    memcpy( ( void * )buffer, ( void * )RFRxBuffer, ( size_t )*size );
}
...
void SX1276LoRaSetTxPacket( const void *buffer, uint16_t size )
{
    ...
    memcpy( ( void * )RFTxBuffer, buffer, ( size_t )TxPacketSize ); 

}

uint32_t SX1276LoRaStartTx(void)
{
	SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
	...
	SX1276WriteFifo( RFTxBuffer, SX1276LR->RegPayloadLength );
...
uint32_t SX1276LoRaSTANDBY(void)
{
...
uint32_t SX1276LoRaStartCAD(void)
{
	SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
...
uint32_t SX1276LoRaStartRx(void)
{
	SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
	...
	memset( RFRxBuffer, 0, ( size_t )RF_BUFFER_SIZE );  //清空RFBuffer
...
uint32_t SX1276LoRaRXDONE(void)
{
	...
	if( LoRaSettings.RxSingleOn == true ) // Rx single mode
	{
		...
		if( LoRaSettings.ImplicitHeaderOn == true )
		{
			...
			SX1276ReadFifo( RFRxBuffer, SX1276LR->RegPayloadLength );
		}
		else
		{
			SX1276Read( REG_LR_NBRXBYTES, &SX1276LR->RegNbRxBytes );
			RxPacketSize = SX1276LR->RegNbRxBytes;
			SX1276ReadFifo( RFRxBuffer, SX1276LR->RegNbRxBytes );
		}
	}
	else // Rx continuous mode
	{
...
uint32_t SX1276LoRaCADRUNNING(void)
{
	if( DIO3 == 1 ) //CAD Done interrupt
	{
		...
		else
		{
			...
			return RF_CHANNEL_EMPTY;
		}
	}
	return RF_BUSY;
}

uint32_t SX1276LoRaTxRxRUNNING(void)
{
	if( DIO0 == 1 ) // TxDone
	{
        if (RFLRState == RFLR_STATE_RX_RUNNING) // RxDone
        {
//            RxTimeoutTimer = GET_TICK_COUNT( );
...
uint32_t SX1276LoRaTxRXFHSSChangedChannel(void)
{
	if( DIO2 == 1 ) // TxDone
	{
