...
void SX1276LoRaStartRx( void )
{
	...
	memset( RFRxBuffer, 0, ( size_t )RF_BUFFER_SIZE );
...
uint32_t SX1276LoRaTxRxRunnin( void )
{
	if( DIO0 == 1 ) // TxDone
	{
		if (RFLRState == RFLR_STATE_TX_RUNNING)
		{
			// Clear Irq
		...
		else if (RFLRState == RFLR_STATE_RX_RUNNING)
		{
            RxTimeoutTimer = GET_TICK_COUNT( );
            if( LoRaSettings.FreqHopOn == true )
            {
